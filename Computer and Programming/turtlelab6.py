...
class Task:
    """
    TASK: help guide Turtle back home while stop to visit Rocky and
    Flippy Rocky's, Flippy's, and Turtle's home will randomly appear
    ANYWHERE on the map.
    """
    def start(self):
        stage.reset()
        ...
        if INTERACTIVE:
            ...
            shuffle(locations)
            (rocky_x,rocky_y),(flippy_x,flippy_y) = locations
        else:
        ...
        stage.add_object(Road(rocky_x,rocky_y,flippy_x,flippy_y,5,"yellow"))
        ...
        return stage.turtle,self.home,self.rocky,self.flippy

    def shortest_path(self):
        dist1 = ( distance(0,0,self.rocky.x,self.rocky.y) +
...
task = Task()
turtle,home,rocky,flippy = task.start()
mod.check = task.done
mod.turtle = turtle
mod.home = home
mod.rocky = rocky
